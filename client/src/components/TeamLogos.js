...
function TeamLogos({ team, logo }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const flaggedTeams = /Ducks|Kings|Lady Ducks/gi; // Regular Expressions / RegEx --> Multiple Ducks and Kings teams that are called Ducks(1), Kings (2) or Lady Ducks for example and if that's the case we just want to extract the Team Name without the corresponding number associated with the team because all the Ducks teams use the same logo and all the Kings teams use the same logo.
        const matchedFlagTeam = team.match(flaggedTeams); // If the destructured team prop passed from parent contains either Ducks or Kings or Lady Ducks teams then this will return an array such as ['Kings'] or ['Ducks'] or ['Lady Ducks']
